...
var smtpWrapper = {
  ...
  init(config) {
    var {comm}               = config
    var cfg                  = config.wrappers.smtp

    if (comm.mode == 'stub')
      this.api = createTransport(require('nodemailer-stub-transport')())
    else
      this.api = createTransport(cfg.wrappers.smtp)
...
var sesWrapper = {
  ...
  init(config) {
    ...
    if (comm.mode == 'stub')
      ...
      this.api = createTransport(sesTransport(cfg.wrappers.ses))
    }
