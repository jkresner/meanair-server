module.exports = function(app, cfg, plumber, ready) {
  ...
  if (api)         app.API = require('./app.api')(api, app, mw)

  if (cfg.auth) {
    mw.cache('setReturnTo', this.session.remember('query.returnTo'))
    mw.cache('authd', this.res.forbid('anon', user=>!user))
    mw.cache('inflateMe', this.data.recast('user','user._id',{required:false}))
  }
  ...
  if (cfg.middleware.session) {
    ...
    var sessionOpts = assign({session}, cfg.middleware.session, {store,
      restrict: req => req.ctx.bot||req.ctx.dirty})
