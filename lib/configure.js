...
module.exports = function(app, env, dotEnv) {
  ...
  //-- addComputerValues() {
  var {about,templates,logic,middleware,model,wrappers,routes} = cfg
  ...
  if (middleware) middleware.dirs = mapDirs(middleware.dirs||'mw', 'server')
  if (model)    model.dirs     = mapDirs(model.dirs||'model', 'server')
  if (model)    model.dir      = model.dirs[0] // temporary for v0.6.1+v0.6.2
  if (templates)
    for (var type in templates.dirs||{})
      templates.dirs[type]     = mapDirs(templates.dirs[type], 'server')
  ...
  if (middleware && middleware.session) {
