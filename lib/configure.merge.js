...
module.exports = (env, $logIt) => ({
  ...
  merge(app, defaults) {
    ...
    var {http} = cfg
    if (http && http.security)
      for (var pattern in http.security.bots || {}) {
        if (typeof http.security.bots[pattern] === 'string')
          http.security.bots[pattern] = new RegExp(http.security.bots[pattern], 'i')
      }
