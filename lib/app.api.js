...
module.exports = (apiConfig, app, mw) => {
...
var endpoints = function(logic, method) {
  ...
  return (opts, cfg) => {
    ...
    for (var fnName in cfg) {
      ...
      $logIt('cfg.route', `api   ${method.toUpperCase()}`, `${this.baseUrl}${url}`)
    }
    return this
  }
}
...
class appApi {
  ...
  constructor(logic, opts) {
    ...
    this.baseUrl = this.opts.baseUrl || `${apiConfig.baseUrl}/${logic}`
    this.router = app.Router(['noBot'])
  ...
  middleware(cfg) { return this.uses(cfg) }
  uses(cfg) {
    if (cfg.constructor === String) cfg = cfg.split(' ')
