...
Instrument.prototype.logIt = function() {
  ...
  if (shouldLogIt) {
    ...
    args[0] = pretty.format(args[0], 24, scope)
    args.unshift(pretty.format(namespace.toUpperCase(), 5, scope))
    this._stdout.write(util.format.apply(this, args) + '\n')
  }
};
...
Instrument.prototype.timed = function(label, msg) {
  ...
  this.log('%s%s%s%s',
    pretty.format(label.toUpperCase(), 6, 'app.init'),
    pretty.format(now - (start||now), 6, 'app.lapse'),
    pretty.format(now - (lastStamp||now), 5, 'app.sublapse'),
...
Instrument.prototype.request = function(req, e, opts) {
  ...
  return util.format("%s %s", pretty.request(req, opts), e?pretty.error(e):'')
