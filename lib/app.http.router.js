module.exports = function(app, express) {
  ...
  return function(name, opts) {
    ...
    var routeIt = method => function() {
      ...
      var startIdx = 0
      // for (var m of chain.params) args.splice(++startIdx,0,m)
      ...
      var mthd = method.toUpperCase()
      while (mthd.length < 5) mthd = mthd + ' '
      $logIt('cfg.route', `${type}${mthd}${mountUrl.replace('/','')}`, `:: ${args[0]}`)
      // console.log(`route.${name}`.white, args, chain.params.length)
    ...
    var honeyRoutes = {
      ...
      param(name, middleware) {
        // chain.params = chain.params.concat(middleware)
        _r.param(name, middleware)
