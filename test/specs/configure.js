...
module.exports = () => DESCRIBE("Config", function() {
  ...
  IT('Default config with barest env variables', function() {
    ...
    process.env.HTTP_SESSIONSTORE_COLLECTION = 'sessions-test2'
    process.env.MONGOURL = 'mongo://ghtest2/db'
    ...
    expect(conf2.auth.oauth.github.passReqToCallback).to.equal(true)
    ...
    expect(conf2.http.static.dir).to.equal('web/public')
    expect(conf2.http.sessionStore.collection).to.equal('sessions-test2')
    expect(conf2.mongoUrl).to.equal('mongo://ghtest2/db')
    DONE()
  })
  ...
  IT('Undefine auth config section from overrides', function() {
    ...
    process.env.HTTP_SESSIONSTORE_COLLECTION = 'sessions-test3'
    process.env.MONGOURL = 'mongo://ghtest3/db'

    var conf3 = Configure('dev', overrides3)
    expect(conf3.auth).to.be.undefined
    expect(conf3.http.host).to.equal('http://localhost:3333')
    expect(conf3.http.sessionStore.collection).to.equal('sessions-test3')
    expect(conf3.mongoUrl).to.equal('mongo://ghtest3/db')
    DONE()
  })


  IT('Works with config logging on', function() {
    var overrides0 = { auth: undefined }
    process.env.LOG_CONF_THEME_LOAD = 'white'
    process.env.HTTP_SESSIONSTORE_COLLECTION = 'sessions-test0'
    process.env.MONGOURL = 'mongo://ghtest0/db'
  ...
  IT('Applies override values on top of default config', function() {
    var overrides4 = { auth: { oauth: { github: { signup: false } } }, mongoUrl: undefined }
    ...
    process.env.HTTP_SESSIONSTORE_COLLECTION = 'sessions-test4'
    ...
    expect(conf4.auth.oauth.github.passReqToCallback).to.equal(true)
  ...
  IT('Add nested override where undefined default', function() {
    var overrides5 = { log: { test5: { theme: { run: 'white', error: 'red' } } }, auth: undefined, mongoUrl: undefined }
    process.env.HTTP_SESSIONSTORE_COLLECTION = 'sessions-test5'
  ...
  IT('Applies env var on top of override', function() {
    var overrides6 = { log: { test6: { theme: { run: 'blue', error: 'magentra' } } }, auth: undefined, mongoUrl: undefined }
    process.env.HTTP_SESSIONSTORE_COLLECTION = 'sessions-test5'
